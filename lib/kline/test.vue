...
<template>
  <div class="container">
    <div class="legend">
      <span>High: {{ candleInfo.high || '-' }}</span> 
      <span>Open: {{ candleInfo.open || '-' }} </span>
      <span>Low: {{ candleInfo.low || '-' }} </span>
      <span>Close: {{ candleInfo.close || '-' }} </span>
      <a-input v-model="inputNum"></a-input>
      <a-button @click="onClick">{{ (inputNum || 1) * 150}}条数据</a-button>
    </div>
    
    <div id="chartContainer" class="main" style="width: 1800px; height: 500px;">
    </div>
  </div>
...
export default {
  ...
  methods: {
    onClick() {
      const myTestData = this.getTestData(this.inputNum);
      console.log('myTestData---', myTestData.length);
      this.chartSeries.setData(myTestData)
    },  
    getTestData(num = 1) {
      const myData = [
        ...
        { time: '2019-05-25', open: 192.54, high: 193.86, low: 190.41, close: 193.59 },
      ];
      const curDate = new Date();
      let resultData = [];
      curDate.setDate(curDate.getDate() - 150 * num);
      for (let j = 0; j < num; j++) {
        for (let i = 0; i < 150; i++) {
          curDate.setDate(curDate.getDate() + 1);
          resultData.push({
            ...myData[i],
            time: this.formatDate(curDate),
          })
        }
      }
      return resultData;
    ...
    initChart() {
      var chart = createChart(document.getElementById('chartContainer'), {
        width: 1500,
        height: 400,
        layout: {
          background: {
      ...
      const myData = this.getTestData(1);
      this.chartSeries.setData(myData);

      chart.subscribeCrosshairMove((param) => {
        if (param.time) {
          const data = param.seriesData.values();
          for (let item of data) {
            this.candleInfo = item; // TODO
          }
        }else {
          this.candleInfo =  {}
        }
